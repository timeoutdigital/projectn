...
class invoiceTaskMovieTest extends invoiceTaskBase
{
    public function testEventInvoice()
    {
        ...
        $days[] = array( 'date' => '2010-07-16', 'expectedNonBillable' => 1, 'expectedInvalidCategory' => 0 );
        ...
        foreach( $days as $k => $day )
        {
            ...
            $xml = simplexml_load_file( $this->options['path'] . 'export_'.date( 'Ymd', $date ).'/movie/test_city.xml' );
            ...
            $this->assertEquals( count( $xml->xpath( '//movie' ) ), $sheet[ $k ]['PROVIDED'] );

            // Check that erroneous and duplicate nodes are not included as 'NEW' (and therefore not billed for).
            $this->assertEquals( count( $xml->xpath( '//movie' ) ) - $day['expectedNonBillable'], $sheet[ $k ]['NEW'] );

            // Check that there is a duplicate node in the XML test file.
            $this->assertEquals( 2, count( $xml->xpath( '//movie[@id="EXA000000000000000000000000000001"]' ) ), 'Please include a duplicate in your test data.' );

            // Check that each UI Category mapped exactly once only, and always to 'Film'.
            // Check that the duplicate node was only included once in the report as 'new'.
            $this->assertEquals( count( $xml->xpath( '//movie' ) ) - $day['expectedNonBillable'], $sheet[0][ 'Film' ] );
