...
<ul class="backendTabs">
<?php foreach ($tabs as $status => $text): ?>
   <li<?php echo ($list == $status) ? ' class="selected"' : ''?>><?php echo link_to($text, 'venue/chooseList?list=' . $status); ?></li>
<?php endforeach ?>
</ul>
...
<?php
    $filters = $sf_user->getAttribute( 'venue.filters', array(), 'admin_module' );
    //var_dump($filters);
?>